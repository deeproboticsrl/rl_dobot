...
import os
import numpy as np
from rl_dobot.learn import DQN_LEARN
...
epsilon_max = 1.0
epsilon_min = 0.1
max_episolon_step = 5000000
dqn = DQN_LEARN(writer=writer,reward_scale=args.reward_scale,discount_factor=args.discount_factor,action_dim=action_dim,
                state_dim=state_dim,target_update_interval=args.target_update_interval,lr=args.lr,)


def epsilon_greedy_action(action, step)
    epsilon = max(epsilon_min, epsilon_max - (epsilon_max - epsilon_min) * step / max_episolon_step)
    if np.random.rand() < epsilon:
        return env.action_space.sample() - 1
    return action


step = 1    # CAUTION: step should not be zerp
max_reward = -np.inf
rewards=[]
for i in range(args.epochs):
    ...
    if i % args.save_iter == 0:
        ...
        dqn.save_model(env_name=args.env_name, info=str(cur_episode))
        buffer.save_buffer(info=args.env_name)

    dqn.writer.add_scalar("Episode Reward", episode_reward, cur_episode)
